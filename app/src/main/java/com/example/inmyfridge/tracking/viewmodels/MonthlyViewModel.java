...
public class MonthlyViewModel extends AndroidViewModel {
    ...
    public void decreaseDisplayedMonths(int months){
        displayedDate = displayedDate.minusMonths(months);
    }

    public void increaseDisplayedMonths(int months){
        displayedDate = displayedDate.plusMonths(months);
    }

    public boolean displayedMonthIsCurrent(){
        return displayedDate.getMonthValue() == currentDate.getMonthValue() && displayedDate.getYear() == currentDate.getYear();
    }

    public MonthlyMacros calculateMacrosForMonth(){
        double calories = 0;
        double protein = 0;
        double carbs = 0;
        double fats = 0;
        ...
        int entries = 0;
        while(tempDate.isAfter(displayedDate.minusMonths(1))){
        ...
        return new MonthlyMacros(calories, protein, fats, carbs, entries);
    }
    ...
    public class MonthlyMacros{

        public MonthlyMacros(double totalCalories, double totalProtein, double totalFats, double totalCarbs, int totalDays) {
